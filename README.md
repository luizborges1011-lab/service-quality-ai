...
Conversation data comes from the firm's own WhatsApp Business account, extracted through its integration with Digisac, the platform used to manage customer conversations across all departments.
...
Acknowledgments
Built on top of Digisac, the WhatsApp Business platform used by ECM Automações
Evaluation powered by OpenAI's GPT-4o via the OpenAI API
